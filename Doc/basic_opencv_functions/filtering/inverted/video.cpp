...
int main(int argc, char* argv[])
{
    ...
    String window_name_of_inverted_video = "inverted Video";

    // Create a window with above names
    namedWindow(window_name_of_original_video, WINDOW_NORMAL);
    namedWindow(window_name_of_inverted_video, WINDOW_NORMAL);
    ...
    while (true)
    {
        ...
        Mat frame_inverted;
        bitwise_not(frame,frame_inverted);

        //show the frames in the created windows
        imshow(window_name_of_original_video, frame);
        imshow(window_name_of_inverted_video, frame_inverted);
